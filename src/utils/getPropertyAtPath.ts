...
export function getPropertyAtPath<T extends object, P extends [LooseAutocompleteB<string | number, "" | 0>?, ...LooseAutocompleteB<string | number, "" | 0>[]]>(
    obj: T,
    path: P
): P extends PropertyPaths<T> | [] ? GetPropertyValueAtPath<T, P>
: IncludesNever<ConstrainPathToObjectPropertyPaths<T, P>> extends true ? undefined
...
>(obj: T, path: P): P extends PropertyPaths<T> ? GetPropertyValueAtPath<T, P> : undefined */ return path.length === 0 ?
            (obj as any)
        :   ((path as any[]).reduce((acc: any, key: any): any => acc[key as never] as any, obj as any) as any);
}
