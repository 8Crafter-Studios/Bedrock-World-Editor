...
/**
 * A regular expression that matches property identifiers.
 */
export const propertyIdentifierRegex = new RegExp(
 ...
 */
export function stringifyNonUniqueSymbol(symbol: symbol): string | undefined {
...
/**
 * Options for the {@link stringifyError} function.
 */
export interface StringifyErrorOptions {
    /**
     * The maximum allowed depth of the cause stringification.
     *
     * @default 10
     */
    maxCauseDepth?: number;
    /**
     * The current depth of the cause stringification.
     *
     * @internal
     *
     * @default 0
     */
    __currentCauseDepth?: number;
}
 ...
 *
 * The error returned by this is intended to be displayed to the user.
 *
 * @param error The error to convert.
 * @param options The options for the stringification.
 * @returns The string representation of the error.
 */
export function stringifyError(error: unknown, options: StringifyErrorOptions = {}): string {
    if (error instanceof Error) {
        if (error.stack && !error.stack.startsWith(error.toString())) {
            return `${error.toString()}\n${error.stack}${error.cause !== undefined ? `\nCaused by: ${stringifyError(error.cause, { ...options, __currentCauseDepth: (options.__currentCauseDepth ?? 0) + 1 })}` : ""}`;
        }
        return `${error.stack ?? error.toString()}${error.cause !== undefined ? `\nCaused by: ${stringifyError(error.cause, { ...options, __currentCauseDepth: (options.__currentCauseDepth ?? 0) + 1 })}` : ""}`;
    }
    switch (typeof error) {
        case "object":
            ...
            if (
                ...
                !Object.prototype.propertyIsEnumerable.call(error, "name") &&
                ...
                if (
                    ...
                    if (!error.stack.startsWith(`${error.name}: ${error.message}`)) {
                        return `${error.name}: ${error.message}\n${error.stack}${"cause" in error && Object.hasOwn(error, "cause") && error.cause !== undefined ? `\nCaused by: ${stringifyError(error.cause, { ...options, __currentCauseDepth: (options.__currentCauseDepth ?? 0) + 1 })}` : ""}`;
                    }
                }
                return error.stack;
            }
            if (
                ...
                return `${error.name}: ${error.message}${"cause" in error && Object.hasOwn(error, "cause") && error.cause !== undefined ? `\nCaused by: ${stringifyError(error.cause, { ...options, __currentCauseDepth: (options.__currentCauseDepth ?? 0) + 1 })}` : ""}`;
            }
            try {
                try {
                    if (typeof JSONB !== "undefined") return JSONB.stringify(error);
                } catch {}
                return JSON.stringify(error);
            } catch {
                // eslint-disable-next-line @typescript-eslint/no-base-to-string -- There is no other option left at this point.
                return String(error);
            }
        case "bigint":
            return `${error}n`;
        ...
        case "function":
            try {
                if (typeof JSONB !== "undefined") return JSONB.stringify(error, undefined, 0, { function: true });
            } catch {}
            return String(error);
        case "symbol":
            return stringifyNonUniqueSymbol(error) ?? error.toString();
        default:
            return String(error);
    }
}
