 ...
 * The maps tab.
 *
 * This tab is used to manage maps in the world.
 *
                    ...
                    {...ConfigConstants.views.Maps.mapsTabModeToSectionIDs[mode].map(
                        (sectionID: (typeof ConfigConstants.views.Maps.mapsTabModeToSectionIDs)[typeof mode][number], index: number): JSX.Element => {
                            function Test1(): JSX.Element {
                                ...
                                // eslint-disable-next-line @typescript-eslint/restrict-template-expressions -- There is only one section atm, if another section is ever added, remove this disable comment.
                                const sectionMode: ConfigConstants.views.Maps.MapsTabSectionMode = sectionID === null ? mode : `${mode}_${sectionID}`;
                                return (
                                    <>
                        ...
                        onClick={(): void => {
                            try {
                                ...
                                for (const key in queryData) {
                                    ...
                                    if (
                                        [...getKeywordedOperators(["dimension", "nbt", "id", "parentid", "locked", "scale", "contents"])].includes(key as never)
                                    ) {
                                        continue;
                                    }
                                    if (
