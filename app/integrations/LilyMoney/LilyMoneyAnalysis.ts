...
import {
    ...
    type LilyMoneyEventEffect,
    type ParsedLilyMoneyEvent,
...
import {
    parseLilyMoneyPendingJobState,

    type LilyMoneyPendingState,
} from "./LilyMoneyPending";
...
export interface LilyMoneyPlayerAnalysis {
    ...
    unexplainedEarnedCents:
        number;

    unexplainedLostCents:
        number;

    anomalyCount: number;

    pendingJobRewardCents:
        number;

    provisionalBalanceCents:
        number | null;
...
function getExpectedActivePendingRaw(
    discovery:
        LilyMoneyDiscoveryResult
): string | null {
    const candidates =
...
export function analyzeLilyMoneyDatabase(
    ...
    for (const event of events) {
        ...
        if (!event.valid) {
            for (
                ...
                errors.push(
                    `Record #${event.record.id} ${event.type}: ${error}`
                );
            }
        }
    }
    ...
    for (
        ...
        if (
            anomaly.differenceCents >
            0
        ) {
            player.unexplainedEarnedCents +=
                anomaly.differenceCents;
        } else {
            player.unexplainedLostCents +=
                -anomaly.differenceCents;
        }
    }
    ...
    errors.push(
        ...pendingJobs.errors.map(
            (error: string): string =>
                `Pending JOB state: ${error}`
        )
    );

    warnings.push(
        ...pendingJobs.warnings.map(
            (
                warning: string
            ): string =>
                `Pending JOB state: ${warning}`
        )
    );
    ...
    for (
        ...
        player.pendingJobRewardCents =
            pending
                .provisionalAmountCents;

        player.pendingRewardGroups =
            pending
...
export function formatLilyMoneyCents(
    cents: number | null
): string {
    if (cents === null) {
        return "—";
