    ...
    public getMesh(
        ...
        for (const variant of variants) {
            ...
            if (variant.x || variant.y) {
                ...
                t.multiply(new THREE.Matrix4().makeRotationY(-(variant.y ?? 0) * Math.PI / 180))
                t.multiply(new THREE.Matrix4().makeRotationX(-(variant.x ?? 0) * Math.PI / 180))
                t.multiply(new THREE.Matrix4().makeTranslation(-8, -8, -8))
        ...
        const scaleMatrix = new THREE.Matrix4().makeScale(0.0625, 0.0625, 0.0625)
        return mesh.transform(scaleMatrix)
    }
