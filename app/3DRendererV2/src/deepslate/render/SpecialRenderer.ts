...
export namespace SpecialRenderers {
    ...
    export function dragonHeadRenderer(texture: Identifier = Identifier.create("enderdragon/dragon")) {
        return (atlas: TextureAtlasProvider) => {
            ...
            transformation.multiply(new THREE.Matrix4().makeTranslation(8, 8, 8));
            transformation.multiply(new THREE.Matrix4().makeScale(0.75, 0.75, 0.75));
            transformation.multiply(new THREE.Matrix4().makeRotationY(Math.PI));
            transformation.multiply(new THREE.Matrix4().makeTranslation(-8, -11.2, -8));
            return new BlockModel(
                undefined,
                {
                    0: texture.withPrefix("entity/").toString(),
    ...
    function createBannerRenderer(color: string, config: { base: any[]; pattern: (index: number) => any }) {
        return (atlas: TextureAtlasProvider, patterns?: NbtList<NbtCompound>) => {
            ...
            return new BlockModel(undefined, textures, elements).getMesh(atlas, Cull.none(), (index: number) => DyeColors[colors[index]!]!);
        };
    }

    export const bannerRenderer = (color: string) =>
        createBannerRenderer(color, {
            base: [
                {
                    from: [-2, -8, 6],
                    to: [18, 32, 7],
    ...
    export const wallBannerRenderer = (color: string) =>
        createBannerRenderer(color, {
            base: [
                {
                    from: [-2, -8, -1.5],
    ...
    const HangingSignRenderers = new Map(
        WoodTypes.map((type) => [`minecraft:${type}_hanging_sign`, SpecialRenderers.hangingSignRenderer(Identifier.create(type))])
    );
    ...
    const ShulkerBoxRenderers = new Map(
        Object.keys(DyeColors).map((color) => [`minecraft:${color}_shulker_box`, SpecialRenderers.shulkerBoxRenderer(Identifier.create(`shulker_${color}`))])
    );
    ...
    export function getBlockMesh(block: BlockState, nbt: NbtCompound | undefined, atlas: TextureAtlasProvider, cull: Cull): Mesh {
        ...
        if (chestRenderer !== undefined) {
            ...
            t.multiply(new THREE.Matrix4().makeTranslation(8, 8, 8));
            t.multiply(
                new THREE.Matrix4().makeRotationY(facing === "west" ? Math.PI / 2 : facing === "south" ? Math.PI : facing === "east" ? (Math.PI * 3) / 2 : 0)
            );
            t.multiply(new THREE.Matrix4().makeTranslation(-8, -8, -8));
        ...
        if (skullRenderer !== undefined) {
            ...
            const t = new THREE.Matrix4();
            t.multiply(new THREE.Matrix4().makeTranslation(8, 8, 8));
            t.multiply(new THREE.Matrix4().makeRotationY(rotation));
            t.multiply(new THREE.Matrix4().makeTranslation(-8, -8, -8));
        ...
        if (signRenderer !== undefined) {
            ...
            t.multiply(new THREE.Matrix4().makeTranslation(8, 8, 8));
            t.multiply(new THREE.Matrix4().makeRotationY(rotation));
            t.multiply(new THREE.Matrix4().makeScale(2 / 3, 2 / 3, 2 / 3));
            t.multiply(new THREE.Matrix4().makeTranslation(-8, -8, -8));
        ...
        if (shulkerBoxRenderer !== undefined) {
            ...
            t.multiply(new THREE.Matrix4().makeTranslation(8, 8, 8));
            if (facing === "down") {
                t.multiply(new THREE.Matrix4().makeRotationX(Math.PI));
            } else if (facing !== "up") {
                const rotationY = facing === "east" ? Math.PI / 2 : facing === "north" ? Math.PI : facing === "west" ? (Math.PI * 3) / 2 : 0;

                t.multiply(new THREE.Matrix4().makeRotationY(rotationY));
                t.multiply(new THREE.Matrix4().makeRotationX(Math.PI / 2));
            }
            t.multiply(new THREE.Matrix4().makeTranslation(-8, -8, -8));
        ...
        if (block.is("bell")) {
            const t = new THREE.Matrix4();
            t.multiply(new THREE.Matrix4().makeTranslation(8, 8, 8));
            t.multiply(new THREE.Matrix4().makeScale(1, -1, -1));
            t.multiply(new THREE.Matrix4().makeTranslation(-8, -8, -8));
        ...
        if (bannerRenderer !== undefined) {
            const rotation = (getInt(block, "rotation") / 16) * Math.PI * 2;
            const t = new THREE.Matrix4();
            t.multiply(new THREE.Matrix4().makeTranslation(8, 24, 8));
            t.multiply(new THREE.Matrix4().makeRotationY(rotation));
            t.multiply(new THREE.Matrix4().makeScale(2 / 3, 2 / 3, 2 / 3));
            t.multiply(new THREE.Matrix4().makeTranslation(-8, -24, -8));
        ...
        if (wallBannerRenderer !== undefined) {
            ...
            t.multiply(new THREE.Matrix4().makeRotationY(rotationY));
            t.multiply(new THREE.Matrix4().makeScale(2 / 3, 2 / 3, 2 / 3));
            t.multiply(new THREE.Matrix4().makeTranslation(-8, -23.2, -8));
        ...
        const t = new THREE.Matrix4().makeScale(0.0625, 0.0625, 0.0625);
