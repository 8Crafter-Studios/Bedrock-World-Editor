...
const rotationAxis: { [key in Axis]: THREE.Vector3 } = {
...
const rescaleAxis: { [key in Axis]: THREE.Vector3 } = {
    x: new THREE.Vector3(1, SQRT2, SQRT2),
    y: new THREE.Vector3(SQRT2, 1, SQRT2),
    z: new THREE.Vector3(SQRT2, SQRT2, 1),
};
...
export class BlockModel {
    ...
    public getDisplayTransform(display: Display) {
        ...
        const center = new THREE.Matrix4().makeTranslation(8, 8, 8);
        const uncenter = new THREE.Matrix4().makeTranslation(-8, -8, -8);
        t.multiply(center);

        if (transform?.translation) {
            t.multiply(new THREE.Matrix4().makeTranslation(...transform.translation));
        }
        if (transform?.rotation) {
            t.multiply(new THREE.Matrix4().makeRotationX((transform.rotation[0] * Math.PI) / 180));
            t.multiply(new THREE.Matrix4().makeRotationY((transform.rotation[1] * Math.PI) / 180));
            t.multiply(new THREE.Matrix4().makeRotationZ((-transform.rotation[2] * Math.PI) / 180));
        }
        if (transform?.scale) {
            t.multiply(new THREE.Matrix4().makeScale(...transform.scale));
        }
    ...
    public getElementMesh(e: BlockModelElement, atlas: TextureAtlasProvider, cull: Cull, getTint: (index?: number) => Color) {
        ...
        const addFace = (face: BlockModelFace, uv: UV, pos: number[]) => {
            ...
            quad.setTexture(
                [u0 + uv[r[0]!]!, v0 + uv[r[1]!]!, u0 + uv[r[2]!]!, v0 + uv[r[3]!]!, u0 + uv[r[4]!]!, v0 + uv[r[5]!]!, u0 + uv[r[6]!]!, v0 + uv[r[7]!]!],
        ...
        const t = new THREE.Matrix4();
        if (e.rotation) {
            const origin = new THREE.Vector3(...e.rotation.origin);
            ...
            t.multiply(translateToOrigin);

            const axis = new THREE.Vector3(...rotationAxis[e.rotation.axis]);
            t.multiply(new THREE.Matrix4().makeRotationAxis(axis, (e.rotation.angle * Math.PI) / 180));

            if (e.rotation.rescale) {
                const scale = new THREE.Vector3(...rescaleAxis[e.rotation.axis]);
                t.multiply(new THREE.Matrix4().makeScale(scale.x, scale.y, scale.z));
            }

            t.multiply(translateBack);
        }
