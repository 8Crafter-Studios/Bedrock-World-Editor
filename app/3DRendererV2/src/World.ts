...
import { WorldChunk, type WorldParams, type WorldSize } from "./WorldChunk";
import type { LevelDB } from "@8crafter/leveldb-zlib";
import type { Dimension } from "mcbe-leveldb";
import { BlockTypeFactory, type BlockType } from "../BlockUtils";
...
export class World extends THREE.Group {
    ...
    constructor(leveldb: LevelDB /* seed = 0 */, scene: THREE.Scene, dimension: Dimension = "overworld") {
        super();
        // this.seed = seed;
        this.scene = scene;
        this.chunkQueue = [];
        this.params.leveldb = leveldb;
        ...
        if (this.chunkQueue.length) {
            ...
            if (!this.initialLoadComplete) {
                ...
                if (menuScreen) {
                    ...
                    const startingPlayerPosition = new THREE.Vector3(player.initialPosition.x, player.initialPosition.y, player.initialPosition.z);
                    for (let y = this.chunkSize.height; y > 0; y--) {
                        if (this.getBlock(startingPlayerPosition.x, y, startingPlayerPosition.z)?.block.value.name.value === "minecraft:grass") {
                            startingPlayerPosition.y = y;
                            break;
                        }
                    }

                    player.position.set(startingPlayerPosition.x, startingPlayerPosition.y + 10, startingPlayerPosition.z);
    ...
    getBlockUnderneath(position: THREE.Vector3, playerHeight: number) {
        return this.getBlock(Math.floor(position.x), Math.floor(position.y - playerHeight / 2 - 1), Math.floor(position.z));
    }
    ...
    getVisibleChunks(player: Player): { x: number; z: number }[] {
        ...
        const range = Array.from({ length: this.renderDistance * 2 + 1 }, (_, i) => i - this.renderDistance);
     ...
     */
    addBlock(x: number, y: number, z: number, blockType: BlockType | string) {
        const block: BlockType = typeof blockType === "string" ? this.blockTypeFactory.get(blockType) : blockType;
        ...
        if (chunk && chunk.loaded) {
            ...
            if (block.pointLightSource) {
                // const blockClass = BlockTypeFactory.get(block) as LightSourceBlock;
                const light = new THREE.PointLight(
                    block.pointLightSource.color,
                    block.pointLightSource.intensity,
                    block.pointLightSource.distance,
                    block.pointLightSource.decay
                );
    ...
    removeBlock(x: number, y: number, z: number) {
        ...
        if (blockToRemove?.block.value.name.value === "minecraft:bedrock") {
            return;
        }
        ...
        if (chunk && chunk.loaded) {
            ...
            if (
                aboveBlock &&
                !this.blockTypeFactory.get(aboveBlock.block.value.name.value).hasCollision &&
                aboveBlock.block.value.name.value !== "minecraft:air"
            ) {
    ...
    hideBlockIfNeeded(x: number, y: number, z: number) {
        ...
        if (chunk && chunk.loaded && chunk.isBlockObscured(coords.block.x, coords.block.y, coords.block.z)) {
