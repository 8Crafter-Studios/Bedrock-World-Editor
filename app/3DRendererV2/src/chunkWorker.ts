...
import { RNG } from "./RNG";
import { type WorldParams, type WorldSize } from "./WorldChunk";
import type { LevelDB } from "@8crafter/leveldb-zlib";
import { entryContentTypeToFormatMap, generateChunkKeyFromIndices, type NBTSchemas, type DimensionVectorXZ, type Vector3 } from "mcbe-leveldb";

declare const self: DedicatedWorkerGlobalScope;

const chunkIDToMessagePortMap = new Map<string, MessagePort>();

self.onmessage = async (event: MessageEvent<undefined | { type: "registerMessagePortID"; id: string; port: MessagePort }>): Promise<void> => {
    if (event.data?.type === "registerMessagePortID") {
        // console.log(`Registering port with ID ${event.data.id}.`);
        chunkIDToMessagePortMap.set(event.data.id, event.data.port);
...
export const generateChunk = async (
    // chunkSize: WorldSize,
    // params: WorldParams,
    // leveldb: Pick<LevelDB, "get">,
    chunkMessagePortID: string,
    chunkIndices: DimensionVectorXZ
): Promise<{ data: NBTSchemas.NBTSchemaTypes.SubChunkPrefix; offset: Vector3 }[]> => {
    // console.log(`Generating chunk at ${chunkIndices.x}, ${chunkIndices.z}, using port with ID: ${chunkMessagePortID}.`);
    ...
    const port: MessagePort | undefined = chunkIDToMessagePortMap.get(chunkMessagePortID);

    if (port === undefined) {
        throw new Error(`No message port found for chunk channel ID ${chunkMessagePortID}`);
    }

    async function getLevelDBEntry(key: Buffer): Promise<Buffer | null> {
        return new Promise<Buffer | null>((resolve) => {
            const eventListener = (event: MessageEvent<{ key: Buffer; data: Buffer | null }>) => {
                // console.log("DATA KEYS", event.data.key, key);
                if (Buffer.from(event.data.key).equals(key)) {
                    // console.log(`Received data for key ${key} with ID ${chunkMessagePortID}.`);
                    port!.removeEventListener("message", eventListener);
                    // port!.postMessage({ key, data: event.data.data });
                    resolve(event.data.data);
                }
            };
            port!.addEventListener("message", eventListener);
            port!.postMessage({ key });
            // console.log(`Sent request for data for key ${key} with ID ${chunkMessagePortID}.`);
        });
    }

    const dataKeys = (
        await Promise.all(
            Array.from({ length: 256 }).map(async (_, i) => ({
                rawData: await getLevelDBEntry(generateChunkKeyFromIndices({ ...chunkIndices, subChunkIndex: i }, "SubChunkPrefix")),
                subChunkPrefix: (i << 24) >> 24,
            }))
        )
    ).filter((key): key is { rawData: Buffer; subChunkPrefix: number } => !!key.rawData);

    // if (dataKeys.length > 0) console.log(`Found data at chunk indices ${chunkIndices.x}, ${chunkIndices.z}.`);

    const data = (
        await Promise.all(
            dataKeys.map(async (key) => {
                try {
                    const data: NBTSchemas.NBTSchemaTypes.SubChunkPrefix = await entryContentTypeToFormatMap.SubChunkPrefix.parse(Buffer.from(key.rawData));
                    return { data, offset: { x: chunkIndices.x * 16, y: key.subChunkPrefix * 16, z: chunkIndices.z * 16 } };
                } catch (e) {
                    console.error(e, key);
                }
                return null;
    ...
    chunkIDToMessagePortMap.delete(chunkMessagePortID);
    return data;
};
