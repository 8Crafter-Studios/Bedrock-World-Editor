...
export class Player {
    ...
    initialPosition = new THREE.Vector3(-279, 72, 336);
    ...
    boundsHelper = new THREE.Mesh(new THREE.CylinderGeometry(this.radius, this.radius, this.height, 16), new THREE.MeshBasicMaterial({ wireframe: true }));
    ...
    applyInputs(dt: number, blockUnderneath: NBTSchemas.NBTSchemaTypes.Block) {
        // Normalize the input vector if more than one key is pressed
        if (this.input.length() > 1) {
            this.input.normalize().multiplyScalar(this.isSprinting ? this.maxSprintSpeed : this.maxSpeed);
        }
        ...
        if (this.spacePressed && this.shiftPressed) {
            this.velocity.y = 0;
        } else if (this.spacePressed /*  && this.onGround */) {
    ...
    async playWalkSound(blockUnderneath: NBTSchemas.NBTSchemaTypes.Block) {
    ...
    updateRaycaster(world: World) {
        ...
        if (intersections.length > 0) {
            ...
            if (this.activeBlockId !== BlockID.Air && intersection.normal) {
                ...
                this.blockPlacementCoords = this.selectedCoords.clone().add(intersection.normal);
            }
    ...
    updateCameraFOV() {
        ...
        new TWEEN.Tween(currentFov).to({ fov: targetFov }, 30).easing(TWEEN.Easing.Quadratic.Out).onUpdate(update).start();
    }
    ...
    onKeyDown(event: KeyboardEvent) {
        ...
        switch (event.code) {
            ...
            case "Digit9":
                ...
                document?.getElementById("toolbar-active-border")?.setAttribute("style", `left: ${this.activeToolbarIndex * 11}%`);
                break;
            case "KeyW":
