...
import { blockIdToKey } from "./Block";
...
export default class Game {
    ...
    initScene() {
        ...
        this.sun.castShadow = false /* true */;
        ...
        this.world = new World(/* 0,  */ this.leveldb, this.scene);
        ...
        if (this.player.controls.isLocked) {
            if (event.button === 0 && this.player.selectedCoords) {
                ...
                if (this.player.activeBlockId != null) {
                    ...
                    this.world.addBlock(blockPos.x, blockPos.y, blockPos.z, blockIdToKey[this.player.activeBlockId]);
                }
            }
        }
    }
    ...
    updateSkyColor() {
        ...
        if (cycleTime < cycleDuration / 2) {
            ...
            this.sun.intensity = (cycleTime - cycleDuration / 2) / (cycleDuration / 2); // Sun intensity increases as the night progresses
        }
        ...
        if (cycleTime >= dayStart && cycleTime < sunsetStart) {
            // Day time
            bottomColor = this.dayColor.clone().lerp(this.sunsetColor, (cycleTime - dayStart) / (sunsetStart - dayStart));
        ...
        const sunAngle = ((2 * Math.PI) / cycleDuration) * (cycleTime + cycleDuration / 6); // Calculate the angle of the sun based on the cycle time with a phase shift of T/4
    ...
    draw() {
        ...
        if (triangleCount) {
            triangleCount.innerHTML = `triangles: ${numberWithCommas(this.renderer.info.render.triangles)}`;
        }

        const renderCalls = document.getElementById("render-calls");
        if (renderCalls) {
            renderCalls.innerHTML = `draw calls: ${numberWithCommas(this.renderer.info.render.calls)}`;
        }
        ...
        this.renderer.render(this.scene, this.player.controls.isLocked ? this.player.camera : this.orbitCamera);
