...
type Candidate = {
    block: NBTSchemas.NBTSchemaTypes.Block;
    x: number;
    y: number;
    z: number;
};
...
export class Physics {
    ...
    update(dt: number, player: Player, world: World): void {
        this.accumulator += dt;
        const blockUnderneath: NBTSchemas.NBTSchemaTypes.Block = this.getBlockUnderneath(player, world)?.block || {
            type: "compound",
            value: {
                name: {
                    type: "string",
    ...
    getBlockUnderneath(player: Player, world: World) {
        return world.getBlock(Math.floor(player.position.x), Math.floor(player.position.y - player.height / 2 - 1), Math.floor(player.position.z));
    }
        ...
        for (let x = minX; x <= maxX; x++) {
            for (let y = minY; y <= maxY; y++) {
                for (let z = minZ; z <= maxZ; z++) {
                    ...
                    if (block) {
                        const blockClass: BlockType = blockTypeFactory.get(block.block.value.name.value);
                        if (!blockClass.hasCollision) {
                            candidates.push({
                                block: block.block,
                                x: x + 0.5,
                                y: y + 0.5,
    ...
    narrowPhase(candidates: Candidate[], player: Player): Collision[] {
        ...
        for (const candidate of candidates) {
            ...
            const closestPoint = new THREE.Vector3(
                Math.max(candidate.x - 0.5, Math.min(player.position.x, candidate.x + 0.5)),
                Math.max(candidate.y - 0.5, Math.min(player.position.y - player.height / 2, candidate.y + 0.5)),
                Math.max(candidate.z - 0.5, Math.min(player.position.z, candidate.z + 0.5))
            );
    ...
    addCollisionHelper(candidate: Candidate) {
        ...
        blockMesh.position.copy(new THREE.Vector3(candidate.x, candidate.y, candidate.z));
